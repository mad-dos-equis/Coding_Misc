...
# true match in right (asymmetric subset case).
#
# CHUNKING: stringdist_inner_join can blow up memory on large inputs because
# intermediate Cartesian products get materialized. We process the left side
# in chunks of `chunk_size` rows. The right side is held constant per chunk.
# A chunk size of ~2000 left rows × 100k right rows is safe on most machines.

match_once <- function(left_dt, right_dt, method, max_dist,
                        match_col = "match_key", chunk_size = 2000L,
                        verbose_chunks = FALSE) {

  n_left <- nrow(left_dt)
  if (n_left == 0) return(data.table())

  # If the left side fits in one chunk, do it directly
  if (n_left <= chunk_size) {
    return(match_once_unchunked(left_dt, right_dt, method, max_dist, match_col))
  }

  # Otherwise, chunk
  chunks <- split(seq_len(n_left), ceiling(seq_len(n_left) / chunk_size))
  if (verbose_chunks)
    cat(sprintf("[chunking %d left rows into %d chunks of ~%d] ",
                n_left, length(chunks), chunk_size))

  results <- vector("list", length(chunks))
  for (i in seq_along(chunks)) {
    if (verbose_chunks && i %% 10 == 0)
      cat(sprintf("[%d/%d] ", i, length(chunks)))
    results[[i]] <- match_once_unchunked(
      left_dt[chunks[[i]]], right_dt, method, max_dist, match_col
    )
  }
  rbindlist(results, fill = TRUE)
}

# Internal worker: single-shot match without chunking.
# Errors are now PROMOTED, not swallowed — silent OOM was the bug source.
match_once_unchunked <- function(left_dt, right_dt, method, max_dist, match_col) {
  m <- fuzzyjoin::stringdist_inner_join(
...
build_silver_standard <- function(left_dt, right_dt,
                                   ...
                                   chunk_size = 2000L,
                                   verbose = TRUE) {
  # NOTE: edit-distance methods (osa, lv) can OOM on large right frames
  # because stringdist materializes intermediate distance matrices.
  # They're excluded from the default tight_configs for that reason.
  # If you have a small right frame and want them included, pass them in.

  preds <- rbindlist(lapply(tight_configs, function(cfg) {
    if (verbose) cat(sprintf("  Running %s @ max_dist=%g ... ",
                             cfg$method, cfg$max_dist))
    p <- tryCatch(
      match_once(left_dt, right_dt, cfg$method, cfg$max_dist,
    ...
    if (is.null(p) || nrow(p) == 0) {
      if (verbose && !is.null(p)) cat("0 matches\n")
      return(data.table())
    }
    if (verbose) cat(sprintf("%d matches\n", nrow(p)))
  ...
  if (verbose) {
    ...
    if (coverage < 0.30)
      cat("  NOTE: low silver coverage. Expected when left and right have\n",
          "  asymmetric coverage (e.g., right is a strict subset).\n")
  }
  silver[]
}
...
evaluate_vs_silver <- function(left_dt, right_dt, silver, method, max_dist,
                                chunk_size = 2000L) {
  preds <- tryCatch(
    match_once(left_dt, right_dt, method, max_dist, chunk_size = chunk_size),
    error = function(e) {
      warning(sprintf("evaluate_vs_silver(%s, %g) failed: %s",
                      method, max_dist, e$message))
      data.table()
    }
  )
...
tiered_match <- function(left_dt, right_dt, tiers, verbose = TRUE,
  ...
  for (i in seq_along(tiers)) {
    ...
    matched <- if (nrow(unmatched) > 0)
      match_once(unmatched, right_dt, t$method, t$max_dist,
                 chunk_size = chunk_size, verbose_chunks = verbose) else data.table()
...
run_fuzzy_match_pipeline <- function(
  ...
  chunk_size = 2000L,        # left rows per chunk for OOM-safe matching
  ...
  matches <- tiered_match(L, R, tiers, verbose = verbose, chunk_size = chunk_size)

  if (verbose) cat("\nStep 5: Generating audit sample ...\n")
  # Audit uses a sample of left rows (the audit doesn't need the full data),
  # so we can call the unchunked path directly for speed.
  audit_left <- if (nrow(L) > 5000) L[sample(.N, 5000)] else L
  audit <- audit_sample(audit_left, R, method = top$method,
...
enrich_left <- function(result, left_df,
  ...
  # Rename for clean output (skip absent so it works on partial match tables)
  setnames(match_info, "tier",         "match_tier",       skip_absent = TRUE)
  setnames(match_info, "method",       "match_method",     skip_absent = TRUE)
  setnames(match_info, "gap",          "match_gap",        skip_absent = TRUE)
  setnames(match_info, "n_candidates", "match_n_cands",    skip_absent = TRUE)
  setnames(match_info, "right_id",     "match_right_id",   skip_absent = TRUE)
  if (!is.na(right_name_y) && right_name_y %in% names(match_info))
    setnames(match_info, right_name_y, "match_right_name", skip_absent = TRUE)
